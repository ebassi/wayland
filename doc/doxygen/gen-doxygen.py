...
def gen_doxygen_file(data, outformat, section, files):
    for l in doxygen_templates[outformat]:
...
if args.output_format == 'man':
    for filename in os.listdir(os.path.join(args.builddir, 'man', 'man3')):
        full_path = os.path.join(args.builddir, 'man', 'man3', filename)
